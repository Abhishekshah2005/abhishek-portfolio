...
export function GradientShimmer({
  ...
  // `as` isn't JSX-callable directly — a lowercase local would be read as a
  // literal tag name ("as"), not looked up as a variable. Aliasing to a
  // capitalized binding is what makes JSX treat it as the dynamic element it
  // is; it also lets `ref` sit in JSX itself instead of a spread props
  // object, which is what react-hooks/refs actually objects to. A generic
  // `ElementType` gives TS nothing concrete to check the call against (fully
  // typing a polymorphic `as` prop is its own well-known rabbit hole), so
  // the props/ref types are deliberately escaped right at this call site —
  // the public prop stays `as?: ElementType`, unaffected.
